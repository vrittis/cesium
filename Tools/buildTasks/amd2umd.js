...
forEachFile('sourcefiles', function(relativePath, file) {
    ...
    for (var zz = 0; zz < sortedNames.length; zz++) {
        contents += "\n    var " + sortedIdentifiers[zz] + " = " + "require(" + sortedNames[zz] + ");";
    }

    var body = result[6];
    body = body.replace(/\s+"use strict";$/m, '');
    var n = body.lastIndexOf('return ');
    body = body.slice(0, n) + body.slice(n).replace('return ', 'module.exports = ');
